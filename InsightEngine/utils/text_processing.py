...
Usadas para limpar saída do LLM, analisar JSON etc.
"""
...
def remove_reasoning_from_output(text: str) -> str:
    ...
    if json_start != -1:
        # Truncar a partir da posição de início do JSON
        return text[json_start:].strip()

    # Se não encontrar marcadores JSON, tentar outros métodos
...
def extract_clean_response(text: str) -> Dict[str, Any]:
    ...
    print(f"Não foi possível analisar resposta JSON: {cleaned_text[:200]}...")
...
def fix_incomplete_json(text: str) -> str:
    ...
    Returns:
        Texto JSON reparado, ou string vazia se não for possível reparar
    """
    ...
    # Verificar se falta o símbolo de abertura de array
    ...
    # Verificar se falta o símbolo de fechamento de array
    ...
    # Reparar colchetes desbalanceados
    ...
    try:
        ...
        # Se ainda for inválido, tentar reparo mais agressivo
...
def fix_aggressive_json(text: str) -> str:
    ...
    if len(objects) >= 2:
        ...
        # Se não encontrar objetos, retornar array vazio
        return '[]'
...
def update_state_with_search_results(search_results: List[Dict[str, Any]],
    ...
    if 0 <= paragraph_index < len(state.paragraphs):
        # Obter a consulta da última busca (supondo que é a consulta atual)
        current_query = ""
        if search_results:
            # Inferir consulta a partir dos resultados de busca (aqui precisa de melhoria para obter a consulta real)
...
def truncate_content(content: str, max_length: int = 20000) -> str:
    ...
    # Tentar truncar no limite de palavras
