"""
Implementação dos nós de busca
...
class FirstSearchNode(BaseNode):
    """Nó que gera a primeira consulta de busca para um parágrafo"""
            ...
            try:
                ...
                if "error" in result:
                    ...
                    if fixed_json:
                        ...
                        logger.error("Não foi possível reparar JSON, usando consulta padrão")
                        return self._get_default_search_query()
    ...
    def _get_default_search_query(self) -> Dict[str, str]:
        ...
        """
        return {
            "search_query": "pesquisa de tema relacionado",
...
class ReflectionNode(BaseNode):
    ...
    def run(self, input_data: Any, **kwargs) -> Dict[str, str]:
        ...
        try:
            ...
            logger.info(f"Consulta de busca gerada por reflexão: {processed_response.get('search_query', 'N/A')}")
            return processed_response

        except Exception as e:
            logger.exception(f"Falha ao gerar consulta de busca por reflexão: {str(e)}")
            raise e
    ...
    def _get_default_reflection_query(self) -> Dict[str, str]:
        """
        Obter consulta de busca padrão para reflexão

        Returns:
            Dicionário de consulta de busca padrão para reflexão
        """
        return {
            "search_query": "informações complementares de pesquisa aprofundada",
            "reasoning": "Devido a falha na análise, usando consulta de busca padrão para reflexão"
        }
