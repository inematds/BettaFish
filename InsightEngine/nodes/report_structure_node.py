...
class ReportStructureNode(StateMutationNode):
    ...
    def run(self, input_data: Any = None, **kwargs) -> List[Dict[str, str]]:
        ...
        Returns:
            Lista de estrutura do relatório
        """
        try:
            ...
            logger.info(f"Geradas {len(processed_response)} estruturas de parágrafo com sucesso")
    ...
    def process_output(self, output: str) -> List[Dict[str, str]]:
        ...
        Returns:
            Lista de estrutura do relatório processada
        """
        try:
                ...
                if "error" in report_structure:
                    ...
                    if fixed_json:
                        try:
                            ...
                            logger.info("Reparo de JSON bem-sucedido")
                        except JSONDecodeError:
                            logger.error("Falha no reparo de JSON")
                            # Retornar estrutura padrão
                            return self._generate_default_structure()
                    else:
                        logger.error("Não foi possível reparar JSON, usando estrutura padrão")
                        return self._generate_default_structure()
            ...
            if not isinstance(report_structure, list):
                ...
                if isinstance(report_structure, dict):
                    # Se for um objeto único, envolver em lista
                    report_structure = [report_structure]
                else:
                    logger.exception("Formato de estrutura do relatório inválido, usando estrutura padrão")
                    return self._generate_default_structure()
            ...
            for i, paragraph in enumerate(report_structure):
                if not isinstance(paragraph, dict):
                    logger.warning(f"Parágrafo {i+1} não está em formato de dicionário, pulando")
                    continue
                ...
                if not title or not content:
                    logger.warning(f"Parágrafo {i+1} sem título ou conteúdo, pulando")
                    continue
            ...
            logger.info(f"Validadas {len(validated_structure)} estruturas de parágrafo com sucesso")
    ...
    def _generate_default_structure(self) -> List[Dict[str, str]]:
        ...
        Returns:
            Lista de estrutura padrão do relatório
        """
        logger.info("Gerando estrutura padrão do relatório")
        return [
            {
                "title": "Visão geral da pesquisa",
                "content": "Visão geral e análise do tema consultado"
            },
            {
                "title": "Análise aprofundada",
                "content": "Análise aprofundada dos diversos aspectos do tema consultado"
            }
        ]

    def mutate_state(self, input_data: Any = None, state: State = None, **kwargs) -> State:
        """
        Escrever a estrutura do relatório no estado
        ...
        try:
            ...
            if not state.report_title:
                state.report_title = f"Relatório de pesquisa aprofundada sobre '{self.query}'"
            ...
            logger.info(f"Adicionados {len(report_structure)} parágrafos ao estado")
            return state

        except Exception as e:
            logger.exception(f"Falha ao atualizar estado: {str(e)}")
            raise e
