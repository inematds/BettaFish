"""
基于章节IR的HTML/PDF渲染器，实现与示例relatorio一致的交互与视觉。
...
class HTMLRenderer:
    ...
    - 读取 IR metadata/chapters，将结构mapeado para响应式HTML；
    - 动态构造Sumario、锚点、Chart.js脚本及互动逻辑；
    - 提供主题变量、编号映射等辅助功能。
    """

    # ===== Guia rapido do fluxo de renderizacao（便于定位注释） =====
    ...
    # _render_body: 组装页面骨架（页眉/header、Sumario/toc、章节/blocks、脚本注水）。
  ...
  <script{defer_attr}>
    ...
    (function() {{
      ...
      if (document.readyState === 'loading') {{
        ...
        setTimeout(checkLib, 100);
            ...
            # 本地Arquivo读取失败，直接使用CDN
            logger.warning(f"{lib_name}本地Arquivo未Encontrado(s)或读取失败，将直接使用CDN")
    ...
    def render(
        ...
        Parametros:
            document_ir: 由 DocumentComposer 生成的整本relatorio数据。
        ...
        # 同步Informacoes estatisticas到本地（用于兼容旧的 _log_chart_validation_stats）
        ...
        title = metadata.get("title") or metadata.get("query") or "智能舆情relatorio"
...
""".strip()
    ...
    def _render_cover(self) -> str:
        ...
        """
        title = self.metadata.get("title") or "智能舆情relatorio"
...
""".strip()
    ...
    def _render_toc_section(self) -> str:
        """
        生成Sumario模块，如无Sumario数据则返回空string。
...
""".strip()
    ...
    def _collect_toc_entries(self, chapters: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        根据metadata中的tocPlan或章节heading收集Sumario项。

        Parametros:
            chapters: Document IR中的章节数组。

        Retorna:
            list[dict]: 规范化后的Sumario条目，包含level/text/anchor/description。
        """
        metadata = self.metadata
        ...
        if custom_entries:
            for entry in custom_entries:
                ...
                if not anchor:
                    logger.warning(
                        f"Sumario项 '{entry.get('display') or entry.get('title')}' "
                        f"缺少有效的anchor，已跳过"
                    )
                    continue
                ...
                if not anchor_valid:
                    logger.warning(
                        f"Sumario项 '{entry.get('display') or entry.get('title')}' "
                        f"的anchor '{anchor}' 在文档中未Encontrado(s)对应的章节"
                    )
    ...
    def _validate_toc_anchor(self, anchor: str, chapters: List[Dict[str, Any]]) -> bool:
        """
        验证Sumarioanchor是否在文档中存在对应的章节或heading。
    ...
    def _extract_embedded_blocks(self, block: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        在block内部查找被误写成string的block列表，并返回补充的block
        """
    ...
    def _format_toc_entry(self, entry: Dict[str, Any]) -> str:
        """
        将单个Sumario项转为带描述的HTML行。

        Parametros:
            entry: Sumario条目，需包含 `text` 与 `anchor`。
    ...
    def _normalize_swot_items(self, raw: Any) -> List[Dict[str, Any]]:
        """将SWOT条目规整为统一结构，兼容string/对象两种写法"""
    ...
    def _normalize_pest_items(self, raw: Any) -> List[Dict[str, Any]]:
        """将PEST条目规整为统一结构，兼容string/对象两种写法"""
        normalized: List[Dict[str, Any]] = []
        if raw is None:
            return normalized
        if isinstance(raw, (str, int, float)):
    ...
    def _apply_cached_review_stats(self, block: Dict[str, Any]) -> None:
        """
        在已审查过的图表上重新累计Informacoes estatisticas，避免重复修复。

        当渲染流程重置了Estatisticas但图表已经审查过（_chart_reviewed=True），
    ...
    def _normalize_chart_block(
        ...
        - 将Erro(s)挂在block顶层的scales合并进props.options。
    ...
    def _ensure_chart_reviewed(
        ...
        *,
        ...
        if not validation_result.is_valid:
            logger.warning(
                f"图表 {block.get('widgetId', 'unknown')} Falha na validacao: {validation_result.errors}"
            )
        ...
        if increment_stats:
            ...
            if validation_result.warnings:
                logger.info(
                    ...
                    f"但有Aviso(s): {validation_result.warnings}"
                )
        block["_chart_review_status"] = "valid"
        block["_chart_review_method"] = "none"
        block["_chart_reviewed"] = True
    ...
    def review_and_patch_document(
        ...
        *,
        ...
        Parametros:
            ...
            reset_stats: 是否重置Estatisticas数据
    ...
    def _render_widget(self, block: Dict[str, Any]) -> str:
        ...
        3. 失败兜底：写入 _chart_renderable=False 及 _chart_error_reason，输出Erro(s)占位而非抛异常。
    ...
    def _log_chart_validation_stats(self):
        """输出图表验证Informacoes estatisticas"""
        stats = self.chart_validation_stats
        if stats['total'] == 0:
            return

        logger.info("=" * 60)
        logger.info("图表验证Estatisticas")
        logger.info("=" * 60)
        logger.info(f"总Quantidade de graficos: {stats['total']}")
    ...
    def _clean_text_from_json_artifacts(self, text: Any) -> str:
        """
        Limpar fragmentos JSON do texto和伪造的结构标记。
...
.llm-error-block {{ /* 含义：LLM Erro(s)提示容器；设置：在本块内调整相关属性 */
...
.toc {{ /* 含义：Sumario容器；设置：在本块内调整相关属性 */
...
@media screen {{
  ...
  .callout.tone-warning {{ /* 含义：Aviso(s)类型 callout；设置：在本块内调整相关属性 */
    --callout-accent: #f59e0b; /* 含义：Aviso(s)橙色调；设置：按需调整数值/颜色/变量 */
    --callout-glow-color: rgba(245, 158, 11, 0.4); /* 含义：Aviso(s)橙发光；设置：按需调整数值/颜色/变量 */
  ...
  .dark-mode .callout.tone-warning {{ /* 含义：暗色Aviso(s)类型；设置：在本块内调整相关属性 */
    --callout-glow-color: rgba(251, 191, 36, 0.5); /* 含义：暗色Aviso(s)发光；设置：按需调整数值/颜色/变量 */
...
"""
    ...
    def _hydration_script(self) -> str:
        ...
        1) 主题切换（#theme-toggle）：监听自定义组件 change 事件，detail 为 'light'/'dark'，
           Funcao:切换 body.dark-mode、刷新 Chart.js 与词云颜色。
...
function hydrateCharts() {
  document.querySelectorAll('canvas[data-config-id]').forEach(canvas => {
    ...
    if (!validation.valid) {
      console.warn('图表数据Falha na validacao:', validation.errors);
      // Falha na validacao但仍然尝试渲染，因为可能会降级成功
    }
...
function exportPdf() {
  ...
  try {
    if (window.pdfFontData) {
      ...
      console.warn('PDF字体数据未Encontrado(s)，将使用默认字体');
    }
  } catch (err) {
