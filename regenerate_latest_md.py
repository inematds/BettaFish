"""
Reencadernar e renderizar relatorio Markdown usando os JSONs de capitulos mais recentes.
"""
...
def render_markdown(document_ir, base_name, timestamp, ir_path=None):
    """
    Renderizar o Document IR em Markdown usando MarkdownRenderer e salvar.

    Apos renderizacao, salva em `final_reports/md`, imprimindo o tamanho do arquivo gerado
    para confirmacao do conteudo de saida.
    ...
    logger.info(f"Markdown gerado com sucesso: {md_path} ({file_size_kb:.1f} KB)")
    return md_path
...
def main():
    """
    Ponto de entrada principal: ler os capitulos mais recentes, encadernar IR e renderizar Markdown.
    ...
    Fluxo:
        ...
        4) Renderizar Markdown e exibir caminho.

    Retorno:
        int: 0 indica sucesso, outros indicam falha.
    """
    logger.info("Reencadernando e renderizando Markdown usando os capitulos LLM mais recentes")
    ...
    logger.info("")
    logger.info("Encadernacao e renderizacao Markdown concluidas")
    logger.info(f"Arquivo IR: {ir_path.resolve()}")
    logger.info(f"Arquivo Markdown: {md_path.resolve()}")
    return 0
